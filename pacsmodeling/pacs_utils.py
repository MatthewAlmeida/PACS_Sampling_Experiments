...
def results_save_filename(arg_ns: Namespace, split:str) -> Path:
    ...
    return Path(
        f"results/{sds_str}/{arg_ns.experiment_name}-cm-{arg_ns.random_seed}-{sds_str}-{split}.pt"
    )
...
def results_tensor_save_filename(arg_ns: Namespace, split:str) -> Path:
    ...
    return Path(
        f"results/{sds_str}/{arg_ns.experiment_name}-cm-tensor-{arg_ns.random_seed}-{sds_str}-{split}-best.pt"
    )
